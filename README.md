...
##  Project Overview
This dbt project turns raw patient measurement data into actionable insights. It helps to track user progress, measure success milestones, and analyze long-term retention.
...
Intermediate/ silver is where we write business logics only for initial bmi of user.

Marts/ gold layer id divided into two part one is fact and dim table and another is answer ready facts(aggregrated fact or snapshot table ) which is build on top of fact and dim .aggregrated fact or snapshot table is where answer ready facts table are stored and is the answer we are looking for. This layer is consumed by analysts for reporting and BI tools and is materialised as a Table.
...
**Dimension Tables:**
- **dim_user:** Patient demographics (Gender, Age, Signup Date) and arrange user on a basis of valid from date to valid to date.  
